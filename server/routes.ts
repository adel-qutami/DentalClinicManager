  ...
  app.get("/api/admin/backup", requireAuth, requirePermission("users_manage"), async (req, res) => {
    try {
      const [
        ...
        storage.getAllUsers(),
        storage.getAllPatientsForBackup(),
        storage.getAllServices(),
        storage.getAllAppointmentsForBackup(),
        storage.getAllVisitsForBackup(),
        storage.getAllPaymentsForBackup(),
        storage.getAllExpensesForBackup(),
